...
norm1 <- ggplot(data=samples) + 
  geom_histogram(aes(x=y1,y=..density..), binwidth = 0.2, colour = "white", fill="cornflowerblue", size = 0.1) + 
  stat_function(fun = function(x) dnorm(x,0,1),color="darkred", size = 1)
norm2 <- ggplot(data=samples) + 
  geom_histogram(aes(x=y2,y=..density..), binwidth = 0.2, colour = "white", fill="cornflowerblue", size = 0.1) + 
  stat_function(fun = function(x) dnorm(x,0,1),color="darkred", size = 1)
