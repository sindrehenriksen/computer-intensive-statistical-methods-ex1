...
ggplot(data = samples) +
  geom_histogram(aes(x=value,y=..density..),binwidth = 0.1, colour = "white", fill = "cornflowerblue", size = 0.1) +
  stat_function(fun = function(x) dexp(x,lambda), color = "darkred", size = 1)
...
lambdas = seq(lambdas_start, lambdas_stop, (lambdas_stop-lambdas_start)/(n_lambdas-1))
